...
export default function AdminReplyReports() {
  const [reports, setReports] = useState<ReplyReport[]>([]);
  const [loading, setLoading] = useState(false);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedReport, setSelectedReport] = useState<ReplyReport | null>(null);
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const { data } = await adminApi.get('/admin-panel/reports/replies/?page=1&status=pending');
      setReports(data.results || data);
    } catch {
      toast.error('Failed to load reply reports');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openActionModal = (report: ReplyReport, type: 'approve' | 'reject') => {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    try {
      await adminApi.patch(`/admin-panel/reports/replies/${selectedReport.id}/`, {
        action: actionType,
  ...
  return (
    <div className="animate-fade-in-up">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[var(--color-primary)]">Reply Reports</h1>
        <p className="text-[var(--color-text-secondary)] mt-1">Review and manage pending reports on replies.</p>
      </div>
        ...
        <div className="card ghost-border overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--color-border-tertiary)] bg-white">
              {reports.map((report) => (
                <tr key={report.id} className="hover:bg-[var(--color-surface-low)] transition-colors">
                  <td className="p-4 font-semibold text-[var(--color-text-primary)] max-w-xs truncate">
                    {report.reply_content || `Reply #${report.id}`}
                  </td>
              ...
              {reports.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-500">No pending reply reports.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[var(--radius-container)] shadow-2xl w-full max-w-md p-6 animate-scale-in">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mb-6">
              You are about to <strong className={actionType === 'approve' ? 'text-green-600' : 'text-red-600'}>{actionType}</strong> the report for this reply.
            </p>
